...
#### 4. 图片文字识别 (OCR)
- **功能**：识别截图或图片中的文字，支持普通文本和 LaTeX 公式。
- **特点**：
    - 集成 RapidOCR (文本) 和 Pix2Text (数学公式)。
    - 支持缩放查看、自定义模型下载。
    - 智能识别中英文及标点符号。

#### 5. ASM to HEX Converter
- **功能**：将汇编代码编译为机器码 (Hex)。
- **特点**：
    - 集成 Keystone Engine，支持 x86/ARM/MIPS 等多架构。
    - 实时/手动汇编模式。
    - 智能错误提示与差异对比。
    - 快捷复制 Hex 字符串、C 数组及转义字面量。

#### 6. HEX to ASM Converter
- **功能**：将机器码 (Hex) 反汇编为汇编代码。
- **特点**：
    - 集成 Capstone Engine。
    - 智能清洗输入（自动去除 `0x`, `,`, 注释等）。
    - 支持多种架构与模式选择。
...
#### 11. Markdown 预览
- **功能**：离线编辑/预览 Markdown，支持 KaTeX 数学公式渲染（`$...$` / `$$...$$`）。
